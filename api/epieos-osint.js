...
// Free-tier keys only support the plain-http endpoint; paid plans can swap
// NUMVERIFY_BASE to https://apilayer.net/api/validate.
//
// POST /api/epieos-osint?provider=numverify   body: { number: "+14158586273" }
// Auth: requires NUMVERIFY_API_KEY env var (server-side only).

const NUMVERIFY_BASE = process.env.NUMVERIFY_BASE || "http://apilayer.net/api/validate";

async function handleNumverifyRequest(req, res) {
  if (req.method !== "POST") return err(res, 405, "POST only");

  const NUMVERIFY_KEY = process.env.NUMVERIFY_API_KEY || "";
  if (!NUMVERIFY_KEY) {
    return err(res, 500, "NUMVERIFY_API_KEY is not configured on the server. Add it in Vercel → Project → Settings → Environment Variables.");
  }

  const { number } = req.body || {};
  const cleanNumber = normalizePhone(number);
  if (!cleanNumber || !PHONE_RE.test(cleanNumber)) {
    return err(res, 400, "Valid phone number required (E.164 format, e.g. +14158586273)");
  }

  const url = `${NUMVERIFY_BASE}?access_key=${encodeURIComponent(NUMVERIFY_KEY)}&number=${encodeURIComponent(cleanNumber.replace(/^\+/, ""))}&format=1`;
  ...
  try {
    ...
    if (e.name === "AbortError") return err(res, 504, "NumVerify request timed out after 15s");
    return err(res, 502, `Failed to reach NumVerify API: ${e.message}`);
  }
  clearTimeout(timeout);
  ...
  if (!payload) return err(res, 502, "NumVerify returned an unreadable response.");

  // NumVerify signals errors via a 200 status with an `error` object in the body.
  if (payload.error) {
    const info = payload.error.info || payload.error.type || "NumVerify lookup failed.";
    return err(res, 400, info, { code: payload.error.code });
  }
  ...
  if (payload.valid === false) {
    return ok(res, 200, {
      number: cleanNumber,
      valid: false,
      reason: "NumVerify could not validate this number (may be malformed or unassigned).",
      scannedAt: new Date().toISOString(),
    });
  }
  ...
  return ok(res, 200, {
    number: cleanNumber,
    valid: payload.valid,
    localFormat: payload.local_format || null,
    internationalFormat: payload.international_format || null,
    countryPrefix: payload.country_prefix || null,
...
export default async function handler(req, res) {
  try {
    ...
    if (req.query?.provider === "numverify") return await handleNumverifyRequest(req, res);
